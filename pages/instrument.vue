    ...
    <v-dialog v-model="dialog" width="400px">
      <v-card>
        <v-card-text>
          ...
          <span class="text-h5 d-flex justify-center mt-2 mb-8">Настройки инструментов</span>
...
export default {
  ...
  methods: {
    ...
    save() {
      if (!!this.model.id) {
        http.api.instrument.update(this.model.id, this.model)
            .then(() => {
              this.getData(this.searchData)
            })
      } else {
        http.api.instrument.save(this.model)
            .then(() => {
              this.getData(this.searchData)
            })
      }
    ...
    getData(search) {
      ...
      http.api.instrument.list(search.rsql, search.page, search.search, search.sort)
          .then(r => {
            this.data = r.content
            this.total = r.total
          })
