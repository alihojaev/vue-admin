...
export default {
  ...
  methods: {
    ...
    save() {
      if (!!this.model.id) {
        http.api.session.update(this.model.id, this.model)
            .then(() => {
              this.getData(this.searchData)
            })
      } else {
        http.api.session.save(this.model)
            .then(() => {
              this.getData(this.searchData)
            })
      }
    ...
    getData(search) {
      ...
      http.api.session.list(search.rsql, search.page, search.search, search.sort)
          .then(r => {
            this.data = r.content
            this.total = r.total
          })
